...
{
    public partial class mudarSenhaView : Form
    {
        public Usuario thisUser { get; set; }
        public mudarSenhaView(Usuario currentUser)
        {
        ...
        private void btnEditarUsuário_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(txtSenha.Text))
            {
                ...
                MessageBox.Show("A nova senha não pode ser nula", "Atenção", buttons, MessageBoxIcon.Warning);
            }
            else if (!txtSenha.Text.Equals(txtConfirmacao.Text)) 
            {
                MessageBoxButtons buttons = MessageBoxButtons.OK;
                MessageBox.Show("A confirmação e a nova senha não podem ser distintas!", "Atenção", buttons, MessageBoxIcon.Warning);
            }
            else
            {
